...
- 주문 & 결제 (유저)
  ...
  - 흐름
    1. 로그인 사용자만 가능
    2. 유저, 상품이 있는지 판단, 없으면 오류 (404 Not Found) 
    3. 상품 있으면 재고 감소(감소시 0 미만이면 400 Bad Request), 없으면 오류(404 Not Found)
    4. 포인트 조회 후 포인트 차감(포인트 차감시 0 미만이면 400 Bad Request) 결제 완료 처리
    5. 결제 완료된 주문 정보 응답, 위에 없는 경우들 404 Not Found
  - 시나리오
